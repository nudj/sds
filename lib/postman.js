...
async function sendIndividualEmail (contact, industry, sample, dryrun) {
  try {
    console.log(`Mailgun: Send to ${contact.email}`)
    ...
    console.error(`Mailgun: Send to ${contact.email} failed`, error)
    return contact
  }
}
...
async function run () {
  try {
    ...
    await companies.each(async company => {
      ...
      console.log(`${dryrun ? '[Dry run]' : ''} Mailgun: Send to ${++i} of ${companies.count} companies succeeded`)
