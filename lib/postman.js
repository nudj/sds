...
const { fetchCampaignLogsByEmail } = require('./utils/arango/helpers')
const mailgun = require('./utils/mailgun')
const stall = require('./utils/stall')
const { isCanadianCompany } = require('./utils/helpers')
...
const getEvents = logs => {
  ...
  // Make events unique by message and event type
  // E.g., Opening one email several times only counts as one "opened" event.
  return _.uniqBy(events, ({ messageId, event }) => `${messageId}${event}`)
}
...
async function sendIndividualEmail (contact, industry, sample, dryrun) {
  try {
    ...
    if (contact.email.endsWith('.ca')) throw 'Avoiding Canadian email address'

    const logCursor = await fetchCampaignLogsByEmail(contact.email)

    const events = getEvents(logCursor)
...
async function sendCompanyEmails (company, companiesBlacklist, dryrun) {
  ...
  try {
    ...
    if (isCanadianCompany(company)) throw 'Avoiding Canadian company'
